...
function openModal(employee = null) {
  if (!canEdit()) {
    Toast.error("You do not have permission to modify employees.");
    return;
  }
  const modal = document.getElementById("formModal");
...
async function deleteEmployee(id) {
  if (!canDelete()) {
    Toast.error("Only Admins can remove employees.");
    return;
  }
  if (!confirm("Remove this employee record? This action cannot be undone."))
    return;
  try {
