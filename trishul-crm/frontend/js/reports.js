...
async function loadReportsPage() {
    let reportsRes, statsRes;
    try {
        [reportsRes, statsRes] = await Promise.all([
            Api.get('/reports'),
    ...
    // Data fetched successfully — render each section independently so a
    // problem in the funnel chart (e.g. Chart.js failing to load) never
    // wipes out the stat cards or the reports list, which already have
    // real data ready to display.
    renderStats(statsRes.data);
    renderReports(reportsRes.data || []);

    if (typeof Chart === 'undefined') {
        console.error('Chart.js failed to load — the funnel chart will be skipped.');
        document.getElementById('funnelChart').closest('.panel').querySelector('.mini-chart-wrap').innerHTML =
            '<div class="empty-state"><i class="fa-solid fa-chart-simple"></i><p>Chart could not load.</p></div>';
    } else {
        try {
