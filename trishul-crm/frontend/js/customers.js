...
function renderTable() {
    ...
    let rows = allCustomers.filter(c => {
        const matchesSearch = !search || [c.name, c.email, c.company].some(v => (v || '').toLowerCase().includes(search));
        ...
        <tr>
            ...
            <td>
                <div class="row-actions">
                    ...
                    <button class="del" onclick="deleteCustomer(${c.id})" title="Delete"><i class="fa-solid fa-trash"></i></button>
                </div>
            </td>
        </tr>
    `).join('');
...
function wireModal() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (!nameInput.value.trim()) {
            nameInput.nextElementSibling?.style && (nameInput.nextElementSibling.style.display = 'block');
            return;
        }
